...
<!------------------------------------FUNCIONES ASIGNAR, FACTURAR y CERRAR TICKET------------------------------------------------------>
					...
					//------ENVIAR EMAIL DE NOTIFICACION DE SERVICIOS------------- 
      						...
      						require("../cnx.php");
							$ssql 		= mysql_query("SELECT * FROM usuarios WHERE id='$cliente3'");			
							$emailCliente	= mysql_result($ssql,0,"email");
							//echo $emailCliente;
				              ...
				              Se ha reportado un nuevo seguimiento a sus su solicitud de servicios. 

				              Por favor ingrese en www.managercode.com/areacliente con su cuenta Tecnico para conocer los detalles del servicio en curso.
				            ...
				            $tecnico    = $_POST['admin'];
					  		if($Tecnico!="Systems Admins")
					  		{

					  			$idTecnico = intval(preg_replace('/[^0-9]+/', '', $tecnico), 10);
					  			require("../cnx.php");
					  			$ssql 		= mysql_query("SELECT * FROM administradores WHERE idAdmin='$idTecnico'");			
								$emailTecnico	= mysql_result($ssql,0,"email");
								//echo $emailTecnico;


				              $to2 = $emailTecnico;
				              $subject2 = 'Nuevo Seguimiento';          
				              $from2 = 'info@mangaercode.com';
				              $headers2 = "From:" . $from2; 
				              $message2 ='
...
/*********************************************ASIGNACION DE TICKETS***************************************************/						
				...
				//------ENVIAR EMAIL DE NOTIFICACION DE SERVICIOS-------------   
				              ...
				              $to = $emailAdmin;
				              $subject = 'Ticket de Servicio Asignado';          
				              $from = 'info@mangaercode.com';
				              $headers = "From:" . $from; 
				              $message ='
				              ...
				              Por favor ingrese en www.managercode.com/areacliente con su cuenta Tecnico para gestionar el servicio solicitado por el cliente.
				                  ';     
				...
				//------ENVIAR EMAIL DE NOTIFICACION DE SERVICIOS-------------   
							...
							$emailCliente	= mysql_result($ssql,0,"email");
				              ...
				              $to = $emailCliente;
				              $subject = 'Factura de Servicios Generada';          
				              $from = 'info@mangaercode.com';
				              $headers = "From:" . $from; 
				              $message ='
				              

				              Se ha cargado una factura a su cuenta Cliente. Puede Verificar los datos desde su cuenta ingresando en www.managercode.com/areacliente 
				              
				              				              
				              Si necesita algun informacion adicional recomendamos comunicarse con nuestro equipo a travez del chat Online en nuestro sitio web www.managercode.com le atenderemos de inmediato. 
